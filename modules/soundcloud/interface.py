...
class ModuleInterface:
    ...
    @staticmethod
    def _extract_raw_date(data):
        raw = ''
        if 'release_date' in data and data['release_date']:
            raw = data['release_date']
        elif 'display_date' in data and data['display_date']:
            raw = data['display_date']
        elif 'created_at' in data and data['created_at']:
            raw = data['created_at']
        raw = raw.split('T')[0] if 'T' in raw else raw
        return raw[:10] if len(raw) >= 10 else raw

    @staticmethod
    def get_release_date(data):
        return ModuleInterface._extract_raw_date(data)

    @staticmethod
    def get_release_year(data):
        raw = ModuleInterface._extract_raw_date(data)
    ...
    def get_track_info(self, track_id, quality_tier: QualityEnum, codec_options: CodecOptions, data={}):
        ...
        return TrackInfo(
            ...
            album_id = '',
            artists = sorted(self.artists_split(metadata['artist'] if metadata.get('artist') else track_data['user']['username'])),
    ...
    def get_album_info(self, album_id, data):
        ...
        return AlbumInfo(
            ...
            release_date = self.get_release_date(playlist_data) or None,
            type = 'ALBUM',
