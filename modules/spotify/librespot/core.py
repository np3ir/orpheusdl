...
class Session(Closeable, MessageListener, SubListener):
    ...
    def reconnect(self) -> None:
        ...
        def _is_auth_failure(exc: Exception) -> bool:
            ...
            return ("badcredentials" in error_text
                    or "bad credentials" in error_text
                    or "login_failed" in error_text)
        ...
        for attempt in range(1, reconnect_attempts + 1):
            try:
                ...
                self.logger.warning("Reconnect attempt {}/{} failed: {}".format(
                    attempt, reconnect_attempts, ex))
                try:
                    if self.connection is not None:
                        self.connection.close()
                finally:
        ...
        raise RuntimeError("Failed to reconnect after {} attempts".format(
            reconnect_attempts)) from last_exception
                    ...
                    if cmd is None:
                        self.__session.logger.info(
                            ...
                            format(util.bytes_to_hex(packet.cmd),
                                   packet.payload))
                        continue
                except (RuntimeError, ConnectionResetError, struct.error,
