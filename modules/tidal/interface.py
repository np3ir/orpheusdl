...
class ModuleInterface:
    ...
    def get_album_info(self, album_id: str, data=None) -> AlbumInfo:
        ...
        if album_id.startswith('videos_'):
            artist_id = album_id.split('_')[1]
            try:
                videos = []
                offset = 0
                while True:
                    resp = self.session.get_artist_videos(artist_id, offset=offset)
                    items = resp.get('items', [])
                    if not items:
                        break

                    videos.extend([v for v in items if v.get('type') == 'Music Video'])

                    total = resp.get('totalNumberOfItems', 0)
                    raw_count = len(items)
                    if offset + raw_count >= total:
                        break
                    offset += raw_count
            except TidalError:
                pass
            ...
            # Create a fake album for videos
            # Use the first video's artist info for the album artist
            first_video = videos[0]
            artist_name = first_video.get('artist', {}).get('name') or "Unknown Artist"
            
            # Cache tracks data for get_track_info usage?
            # get_track_info usually fetches individual track if not in data.
            # We can populate cache to avoid fetching each video again if possible.
            # But get_track_info is called by music_downloader with empty data usually?
            # Actually music_downloader passes `extra_kwargs` which comes from `album_info.track_extra_kwargs`.
            
            cache_data = {}
            for v in videos:
                cache_data[str(v['id'])] = v

            return AlbumInfo(
                name="Music Videos",
                release_year=None,
                explicit=any(v.get('explicit') for v in videos),
                quality=None,
                upc=None,
                duration=sum(v.get('duration') for v in videos),
                cover_url=self._generate_artwork_url(first_video.get('imageId'), size=self.cover_size) if first_video.get('imageId') else None,
                cover_type=ImageFileTypeEnum.jpg,
                artist=artist_name,
                artist_id=artist_id,
                tracks=[str(v['id']) for v in videos],
        ...
        _raw_release_date = album_data.get('releaseDate') or album_data.get('streamStartDate') or ''
    ...
    def get_track_info(self, track_id: str, quality_tier: QualityEnum, codec_options: CodecOptions,
        ...
        try:
            ...
            is_video = False
            # If get_track returns a video object (sometimes happens), it might lack 'album'
            if track_data.get('type') == 'Music Video' or (not track_data.get('album') and track_data.get('imageId')):
                is_video = True
        except TidalError:
            try:
                track_data = self.session.get_video(track_id)
                is_video = True
            except TidalError:
                raise
        ...
        if is_video:
            ...
            album_data = {
                'title': 'Music Video',
                'artist': track_data.get('artist') or (track_data.get('artists')[0] if track_data.get('artists') else None),
                'cover': track_data.get('imageId'),
            ...
            if not album_info:
                # Fallback if album is missing but not detected as video
                album_id = "0"
                album_data = {
                    'title': 'Unknown Album',
        ...
        if not is_video:
            ...
            format = None # Video format handling?
        ...
        try:
            if is_video:
                # Map quality tiers to video quality
                video_quality = {
                    QualityEnum.LOW: 'LOW',
                    QualityEnum.MEDIUM: 'MEDIUM',
                    QualityEnum.HIGH: 'HIGH',
                    QualityEnum.LOSSLESS: 'HIGH',
                    QualityEnum.HIFI: 'HIGH'
                }.get(quality_tier, 'HIGH')
                stream_data = self.session.get_video_stream_url(track_id, video_quality)
            else:
        ...
        if stream_data is not None:
            if stream_data['manifestMimeType'] == 'application/dash+xml':
                ...
                tracks = self.parse_mpd(manifest)
                # Sort by bitrate descending to get best quality
                tracks.sort(key=lambda t: t.bitrate, reverse=True)
                audio_track = tracks[0] if tracks else None
                track_codec = audio_track.codec if audio_track else CodecEnum.FLAC
            else:
                manifest = json.loads(base64.b64decode(stream_data['manifest']))
                if is_video:
                    # Video manifest: {"mimeType": "video/mp4", "urls": ["...m3u8"]}
                    # Parse M3U8 HLS stream like tiddl does
                    m3u8_master_url = manifest.get('urls', [None])[0]
                    if not m3u8_master_url:
                        raise ValueError('No M3U8 URL in video manifest')
                    req_session = create_requests_session()
                    master_resp = req_session.get(m3u8_master_url)
                    master_lines = master_resp.text.splitlines()
                    # Find quality playlist URIs (lines after #EXT-X-STREAM-INF)
                    quality_uris = []
                    for i, line in enumerate(master_lines):
                        if line.startswith('#EXT-X-STREAM-INF') and i + 1 < len(master_lines):
                            quality_uris.append(master_lines[i + 1])
                    # Pick highest quality (last entry)
                    segment_playlist_url = quality_uris[-1] if quality_uris else m3u8_master_url
                    seg_resp = req_session.get(segment_playlist_url)
                    segment_urls = [l for l in seg_resp.text.splitlines() if l and not l.startswith('#')]
                    track_codec = CodecEnum.H264
                    audio_track = AudioTrack(codec=CodecEnum.H264, bitrate=0, sample_rate=0, urls=segment_urls)
        ...
        bit_depth = (24 if stream_data and stream_data.get('audioQuality') == 'HI_RES_LOSSLESS' else 16) \
        ...
        if stream_data and stream_data.get('audioQuality'):
            # fallback bitrate
            bitrate = {
                'LOW': 96,
                'HIGH': 320,
            ...
            if stream_data.get('audioMode') == 'DOLBY_ATMOS':
                ...
                if track_codec == CodecEnum.EAC3:
                    ...
                    bitrate = 256
            elif stream_data.get('audioMode') == 'SONY_360RA':
                bitrate = 667

        # more precise bitrate tidal uses MPEG-DASH
        if audio_track:
            bitrate = audio_track.bitrate // 1000
            if stream_data and stream_data.get('audioQuality') == 'HI_RES_LOSSLESS':
        ...
        elif track_data.get('imageId'): # Video cover
            cover_url = self._generate_artwork_url(track_data.get('imageId'), size=self.cover_size)
        else:
        ...
        artist_id = (track_data.get('artist') or {}).get('id')
        if not artist_id and track_data.get('artists'):
            first_artist = track_data.get('artists')[0]
            if first_artist:
                artist_id = first_artist.get('id')

        _raw_artists = track_data.get('artists') or []
        _main = sorted([a.get('name') for a in _raw_artists if a.get('type') == 'MAIN'])
        _feat = sorted([a.get('name') for a in _raw_artists if a.get('type') == 'FEATURED'])
        _no_type = [a.get('name') for a in _raw_artists if not a.get('type')]
        _ordered_artists = (_main or _no_type) + _feat

        track_info = TrackInfo(
            name=track_name,
            album=album_data.get('title'),
            album_id=album_id,
            artists=_ordered_artists,
            artist_id=artist_id,
        ...
        for period in root.findall('Period'):
            for adaptation_set in period.findall('AdaptationSet'):
                for rep in adaptation_set.findall('Representation'):
                    # Check if representation is audio or video
                    ...
                    if seg_timeline is not None:
                        ...
                        for s in seg_timeline.findall('S'):
                            ...
                            # Number of segments with same duration
                        ...
                        for time in seg_time_list:
                            track_urls.append(media.replace('$Time$', str(time)))

                    else:
                        # addressing-explicit - Example 6
                        # https://dashif-documents.azurewebsites.net/Guidelines-TimingModel/master/Guidelines-TimingModel.html#addressing-explicit
                        duration = int(seg_template.get('duration'))
                        timescale = int(seg_template.get('timescale'))
                        
                        # Calculate total duration from Period duration if available
                        period_duration = period.get('duration')
                        # Parse ISO 8601 duration (PT1M30S etc) - simplified
                        # This is a bit complex, maybe just rely on startNumber?
                        # For now, let's assume we can't easily calculate total segments without duration.
                        # But Tidal usually uses SegmentTimeline.
                        pass
    ...
    def get_track_download(self, file_url: str = None, audio_track: AudioTrack = None) \
        ...
        try:
            # Use codec='copy' to support both audio and video without transcoding
            ffmpeg.input(merged_temp_location, hide_banner=None, y=None).output(output_location, c='copy',
                                                                                loglevel='error').run()
            # Remove all files
            ...
            return TrackDownloadInfo(
                ...
                # If it's a video, this fallback is not ideal as it sets codec to AAC.
                # But if ffmpeg fails, we can't do much. 
                # Assuming standard AAC fallback for audio, for video it might just be renamed.
                different_codec=CodecEnum.AAC if audio_track.codec not in [CodecEnum.H264, CodecEnum.H265] else audio_track.codec
            )
    ...
    def get_track_cover(self, track_id: str, cover_options: CoverOptions, data=None) -> CoverInfo:
        ...
        if not cover_id and track_data.get('imageId'):
            cover_id = track_data.get('imageId')
    ...
    def get_track_lyrics(self, track_id: str, track_data: dict = None) -> LyricsInfo:
        ...
        if 'error' in lyrics_data and track_data:
            ...
            best_tracks = [r.result_id for r in results
                           ...
                           (track_data.get('artist') and r.artists[0] == track_data.get('artist').get('name')) and
    ...
    def get_track_credits(self, track_id: str, data=None) -> Optional[list]:
        ...
        if track_id in data:
            ...
            try:
                contributors_data = self.session.get_track_contributors(track_id)
                track_contributors = contributors_data.get('items') if contributors_data else []
            except TidalError:
                track_contributors = []

            if track_contributors and len(track_contributors) > 0:
                for contributor in track_contributors:
