...
class Downloader:
    ...
    def select_music_videos_from_artist(
        ...
        for music_video in music_videos:
            choice_name = " | ".join([
                self.millis_to_min_sec(music_video["attributes"]["durationInMillis"]),
                f'{music_video["attributes"].get("contentRating", "None").title():<8}',
                music_video["attributes"]["name"],
            ])
            choices.append(Choice(name=choice_name, value=music_video))
    ...
    def get_decryption_key(self, pssh: str, track_id: str, stream_url: str) -> str:
        try:
            full_pssh_bytes = base64.b64decode(pssh)
        except base64.binascii.Error as e_b64:
            print(f"[gamdl Downloader ERROR] Invalid base64 PSSH string: {pssh[:30]}... Error: {e_b64}")
            return self.DEFAULT_DECRYPTION_KEY
        
        pssh_obj_for_challenge = PSSH(full_pssh_bytes)
        
        license_request_uri = None
        if pssh_obj_for_challenge.key_ids and len(pssh_obj_for_challenge.key_ids) > 0:
            first_kid_uuid = pssh_obj_for_challenge.key_ids[0]
            try:
                key_id_hex_str = first_kid_uuid.hex
                key_id_bytes = bytes.fromhex(key_id_hex_str)
                short_b64_kid = base64.b64encode(key_id_bytes).decode('utf-8')
                license_request_uri = f"data:;base64,{short_b64_kid}"
                print(f"[gamdl Downloader DEBUG] Constructed license_request_uri from KID: {license_request_uri}")
            except ValueError as e_val:
                print(f"[gamdl Downloader ERROR] Error processing KID {first_kid_uuid.hex if first_kid_uuid else 'N/A'}: {e_val}. Using full PSSH for URI.")
                license_request_uri = f"data:;base64,{pssh}"
        else:
            print("[gamdl Downloader WARNING] No key_ids in PSSH. Using full PSSH for URI.")
            license_request_uri = f"data:;base64,{pssh}"

        print(f"[gamdl Downloader DEBUG] PSSH object for challenge (system ID: {pssh_obj_for_challenge.system_id if pssh_obj_for_challenge.system_id else 'N/A'}, KIDs: {[kid.hex for kid in pssh_obj_for_challenge.key_ids] if pssh_obj_for_challenge.key_ids else []})")

        try:
            cdm_session = self.cdm.open()
            challenge_b64 = base64.b64encode(
                self.cdm.get_license_challenge(cdm_session, pssh_obj_for_challenge)
            ).decode("utf-8")
            license_b64 = self.apple_music_api.get_widevine_license(
                track_id,
                license_request_uri,
                challenge_b64,
            )
            self.cdm.parse_license(cdm_session, license_b64)
            decryption_key = next(
                i for i in self.cdm.get_keys(cdm_session) if i.type == "CONTENT"
            ).key.hex()
        except Exception as e_license:
            print(f"[gamdl Downloader WARNING] Failed to get decryption key via pywidevine: {e_license}. Falling back to default key.")
            return self.DEFAULT_DECRYPTION_KEY
        finally:
            if 'cdm_session' in locals() and self.cdm:
